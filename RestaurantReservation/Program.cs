...
{
    Console.WriteLine(
        $"- Reservation {reservation.ReservationId} on {reservation.Date:u} for {reservation.PartySize} guests at {reservation.Restaurant.Name}");
}
...
{
    Console.WriteLine(
        $"- Reservation {reservation.ReservationId} for {reservation.CustomerFirstName} {reservation.CustomerLastName} at {reservation.RestaurantName}");
    Console.WriteLine(
        $"- Reservation {reservation.ReservationId} for {reservation.CustomerFirstName} {reservation.CustomerLastName} at {reservation.RestaurantName}");
}

var employeeViewRows = await service.ListEmployeesWithRestaurantAsync();
Console.WriteLine("Employees from view:");
foreach (var employee in employeeViewRows)
{
    Console.WriteLine(
